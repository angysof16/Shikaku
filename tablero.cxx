...
bool cargarTablero(const std::string &archivo, Tablero &tablero){
    ...
    if((!(f >> tablero.filas >> tablero.columnas)) || tablero.filas <= 0 || tablero.columnas <= 0) {
        std::cerr << "Error al leer las dimensiones del tablero o dimensiones no válidas." << std::endl;
        return false;
    }
...
bool mostrarTablero(const Tablero &tablero) {
    ...
    std::cout << "   ";
    for (int j = 0; j < tablero.columnas; j++) std::cout << "---";
    std::cout << "-\n";
...
bool validarTablero(const Tablero &tablero){
    if ((tablero.filas <= 0 || tablero.columnas <= 0) ||((int)tablero.celdas.size() != tablero.filas)) {
        std::cerr << "Tablero no válido para validar." << std::endl;
        return false;
    }
    ...
    for(int i = 0; i < tablero.filas; i++) {
        if((int)tablero.celdas[i].size() != tablero.columnas) return false;

        for(int j = 0; j < tablero.columnas; j++) {
            int valor = tablero.celdas[i][j];
            if (valor < 0) return false;
            if (valor > totalCeldas) return false;
            if (valor > 0) sumaArea += valor;
        }
    }
